...
local function toggleSitSystem()
    if sitEnabled then
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "System", 
            Text = "กำลังทำงานอยู่ รอสักครู่", 
            Duration = 2
        })
        return
    end
    ...
    if not c or not c:FindFirstChild("Humanoid") or not c:FindFirstChild("HumanoidRootPart") then 
        sitEnabled = false
        return 
    end
    
    local originalPos = c.HumanoidRootPart.CFrame
    local originalY = originalPos.Y
    
    c.Humanoid.Sit = true
    
    c.HumanoidRootPart.CFrame = CFrame.new(originalPos.X, originalY + sitHeight, originalPos.Z)
    
    task.spawn(function()
        task.wait(1)
        if c and c:FindFirstChild("HumanoidRootPart") and c:FindFirstChild("Humanoid") then
            c.HumanoidRootPart.CFrame = CFrame.new(originalPos.X, originalY, originalPos.Z)
...
if _ktype == "adminmax" then
    ...
    AdminTab:Section({Title = "ADMIN CONTROL"})

    AdminTab:Button({Title = "Show HWID", Callback = function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "HWID",
            Text = _getHWID(),
            Duration = 5
    ...
    AdminTab:Button({Title = "Reset Key (Logout)", Callback = function()
        pcall(function() writefile("phs.txt", "") end)
        game:GetService("Players").LocalPlayer:Kick("Key reset. Please re-enter key.")
    end})

    AdminTab:Button({Title = "Check Key Status", Callback = function()
        local s = _load()
        if s and s.key then
            local expText = s.expires and os.date("%Y-%m-%d %H:%M:%S", s.expires) or "∞"
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Key Status",
                Text = "Key: " .. s.key .. "\nType: " .. _typeLabel(s.type or "?") .. "\nExpires: " .. expText,
                Duration = 8
            })
        else
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Key Status",
                Text = "No saved key found",
                Duration = 3
            })
        end
    end})

    AdminTab:Button({Title = "Fetch Latest Keys", Callback = function()
        local data = _fetch()
        if data then
            local count = 0
...
PlayerTab:Divider()
PlayerTab:Section({Title = "UNDERGROUND TELEPORT"})
PlayerTab:Button({Title = "Teleport Under (1 sec)", Callback = function() toggleSitSystem() end})
PlayerTab:Slider({Title = "Depth", Step = 0.5, Value = {Min = -10, Max = -2, Default = -2}, Callback = function(v) sitHeight = v end})
...
PlayerTab:Toggle({Title = "Ghost Shake", Default = false, Callback = function(state)
...
PlayerTab:Divider()
PlayerTab:Section({Title = "ANTI DEATH"})
PlayerTab:Toggle({Title = "Anti Death", Default = false, Callback = function(state)
...
VehicleTab:Button({Title = "Pull My Vehicles", Callback = function()
    ...
    if #myVehicles == 0 then
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Vehicle", Text = "No vehicles found", Duration = 3})
        return
    end
    local pulled = 0
    ...
    game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Vehicle", Text = "Pulled " .. pulled .. " vehicles", Duration = 3})
end})

VehicleTab:Slider({Title = "Pull All Radius", Step = 5, Value = {Min = 10, Max = 100, Default = 30}, Callback = function(v) pullAllRadius = v end})
VehicleTab:Button({Title = "Pull Other Players Vehicles", Callback = function()
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    local vehicles = workspace:FindFirstChild("Vehicles")
    if not vehicles then return end
...
VehicleTab:Toggle({Title = "Shake All Vehicles", Default = false, Callback = function(state)
...
VehicleTab:Button({Title = "Repair Current Vehicle", Callback = function()
    local char = LocalPlayer.Character
    if not char then return end
    local seat = char:FindFirstChild("SeatPart") or char:FindFirstChild("SeatWeld")
    if not seat then
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Vehicle", Text = "You are not in a vehicle", Duration = 3})
        return
    end
    local vehicle = seat.Parent
    while vehicle and not vehicle:IsA("Model") do vehicle = vehicle.Parent end
    if vehicle then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Vehicle", Text = "Vehicle repaired", Duration = 3})
    end
end})

VehicleTab:Button({Title = "Clear Nearby Vehicles", Callback = function()
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    local vehicles = workspace:FindFirstChild("Vehicles")
    if not vehicles then return end
    ...
    game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Vehicle", Text = "Cleared " .. cleared .. " vehicles", Duration = 3})
end})
...
ESPTab:Toggle({Title = "Ground Item ESP", Default = false, Callback = function(v) getgenv().ItemESPEnabled = v end})
...
PVPTab:Input({Title = "Safe Friend", Placeholder = "Player name...", Callback = function(v)
    excludedPlayerNames = {}
...
PVPTab:Input({Title = "Admin Players", Placeholder = "Player name...", Callback = function(v)
    adminPlayers = {}
...
CustomTab:Dropdown({Title = "Boost Mode", Values = {"Mode 1", "Mode 2", "Mode 3"}, Default = 1, Callback = function(v)
    fpsBoosterMode = v == "Mode 1" and 1 or v == "Mode 2" and 2 or 3
...
game:GetService("StarterGui"):SetCore("SendNotification", {Title = "PIG HUB ULTIMATE", Text = "Loaded successfully!", Duration = 4})