...
local function _xor(s)
    local r,b = "",bit32.bxor
    for i=1,#s do r=r..string.char(b(string.byte(s,i),_XK)) end
    return r
end

-- URL 1 (ลูกค้า) และ URL 2 (admin) เข้ารหัส XOR
local _E1 = {66,94,94,90,89,16,5,5,88,75,93,4,77,67,94,66,95,72,95,89,79,88,73,69,68,94,79,68,94,4,73,69,71,5,65,65,79,66,69,78,7,71,75,82,5,7,30,24,24,30,31,28,29,18,5,88,79,76,89,5,66,79,75,78,89,5,71,75,67,68,5,65,79,83,89,4,64,89,69,68}
...
local _ALLOWED = {["raw.githubusercontent.com"]=true}
local function _chk(url)
    local d=url:match("https?://([^/]+)")
    return d and _ALLOWED[d]==true
end

local function _fetchOne(enc)
    local url=_dec(enc)
    if not _chk(url) then return nil end
...
-- [[ STORAGE ]] --
local _sf = string.char(112,104,115,46,116,120,116)
local function _save(t) pcall(writefile,_sf,_HS:JSONEncode(t)) end

local function _load()
    local ok,v = pcall(readfile,_sf)
...
local function _fmt(s)
    ...
    elseif m>0 then return m.."น. "..(s%60).."ว."
    else return (s%60).."ว." end
end

local _tl = {["1min"]="1 นาที",["1day"]="1 วัน",["1week"]="1 อาทิตย์",["permanent"]="ถาวร",["admin"]="ADMIN"}
local function _typeLabel(t) return _tl[t] or t end
...
local function _showKey(cb)
    ...
    Sub.Text = "ลงชื่อเข้าใช้ด้วยบัญชี PIGHUB"
    ...
    Btn.MouseButton1Click:Connect(function()
        ...
        if key == "" then
            SL.Text = "กรุณาใส่ Username!" SL.TextColor3 = Color3.fromRGB(255,200,50) return
        end
        SL.Text = "กำลังเชื่อมต่อ..." SL.TextColor3 = Color3.fromRGB(180,150,255)
        task.spawn(function()
            if key == "PIGHUB4/2/55" then
                -- ตรวจ slot admin ไม่เกิน 30
                local data = _fetch()
                local adminSlots = 0
                local myHWID = _getHWID()
                local alreadyIn = false
                if data and data["__admin_slots"] then
                    local slots = data["__admin_slots"]
                    for _,hwid in ipairs(slots) do
                        if tostring(hwid) == tostring(myHWID) then alreadyIn = true end
                        adminSlots = adminSlots + 1
                    end
                end
                if not alreadyIn and adminSlots >= 1 then
                    SL.Text = "มีคนใช้งานอยู่แล้ว" SL.TextColor3 = Color3.fromRGB(255,80,80) return
                end
                _save({key=key,type="admin",expires=nil,hwid=myHWID})
                SL.Text = "ยินดีต้อนรับ!" SL.TextColor3 = Color3.fromRGB(80,255,160)
                task.wait(0.8) G:Destroy() cb("admin",nil) return
            end
            local data = _fetch()
            if not data then
                SL.Text = "เชื่อมต่อ server ไม่ได้" SL.TextColor3 = Color3.fromRGB(255,80,80) return
            end
            local entry = data[key]
            if not entry then
                SL.Text = "ไม่พบบัญชีนี้" SL.TextColor3 = Color3.fromRGB(255,80,80) return
            end
            if not entry.active then
                SL.Text = "กำลังตรวจสอบ..." SL.TextColor3 = Color3.fromRGB(255,80,80)
                task.wait(1.5)
            ...
            if locked ~= "" and locked ~= "null" and locked ~= tostring(hwid) then
                SL.Text = "บัญชีนี้ login จากเครื่องอื่นอยู่" SL.TextColor3 = Color3.fromRGB(255,80,80) return
            end
            ...
            if exp and os.time() > exp then
                SL.Text = "บัญชีหมดอายุแล้ว" SL.TextColor3 = Color3.fromRGB(255,80,80) return
            end
...
task.spawn(function()
                ...
                task.spawn(function()
                    ...
                    task.spawn(function()
                        ...
                        while KG2 and KG2.Parent do
                            ...
                            KL2.Text = "● Online | " .. LocalPlayer.Name
                                KL2.TextColor3 = Color3.fromRGB(80,255,160)
                            else
                                local rem = _kexp - os.time()
                                if rem <= 0 then
                                    KL2.Text = "● Session หมดแล้ว"
                                    KL2.TextColor3 = Color3.fromRGB(255,80,80)
                                    task.wait(3)
                                    game:GetService("Players").LocalPlayer:Kick("Session หมดอายุ กรุณา login ใหม่")
                                    break
                                end
                                KL2.Text = "● Online | " .. _fmt(rem)
    ...
    task.spawn(function()
        ...
        while KG and KG.Parent do
            ...
            if _kexp == nil then
                KL.Text = "● Online | " .. LocalPlayer.Name
                KL.TextColor3 = Color3.fromRGB(80,255,160)
            else
                local rem = _kexp - os.time()
                if rem <= 0 then
                    KL.Text = "● Session หมดแล้ว"
                    KL.TextColor3 = Color3.fromRGB(255,80,80)
                    task.wait(3)
                    game:GetService("Players").LocalPlayer:Kick("Session หมดอายุ กรุณา login ใหม่")
                    break
                end
                KL.Text = "● Online | " .. _fmt(rem)
...
-- [[ ADMIN PANEL ]] --
local _adminLog = {}
local _adminFeatures = {
    WalkSpeed=true,JumpEnabled=true,SitEnabled=true,AntiLook=true,
    GhostShake=true,AntiDeath=true,ESPBox=true,ESPName=true,
    ESPItem=true,SilentAim=true,GunMods=true,VehicleShake=true,
    AntiRam=true,FPSBooster=true,PigHubEffect=true,
}
local function _logAdd(msg)
    table.insert(_adminLog,1,os.date("%H:%M:%S").." "..msg)
    if #_adminLog>50 then table.remove(_adminLog) end
end
...
if _ktype == "admin" then
    task.spawn(function()
        ...
        AG.Name="PHAdmin" AG.ResetOnSpawn=false AG.DisplayOrder=9998

        local ABtn = Instance.new("TextButton",AG)
        ABtn.Size=UDim2.new(0,36,0,36)
        ABtn.Position=UDim2.new(0,12,1,-115)
        ABtn.BackgroundColor3=Color3.fromRGB(180,30,60)
        ABtn.TextColor3=Color3.fromRGB(255,255,255)
        ABtn.Text="⚙" ABtn.TextScaled=true
        ABtn.Font=Enum.Font.GothamBold ABtn.BorderSizePixel=0
        ABtn.Active=true ABtn.Draggable=true
        Instance.new("UICorner",ABtn).CornerRadius=UDim.new(0,8)

        local APanelOpen=false
        local APanel=Instance.new("Frame",AG)
        APanel.Size=UDim2.new(0,340,0,480)
        APanel.Position=UDim2.new(0.5,-170,0.5,-240)
        APanel.BackgroundColor3=Color3.fromRGB(8,6,16)
        APanel.BorderSizePixel=0 APanel.Active=true APanel.Draggable=true
        APanel.Visible=false
        Instance.new("UICorner",APanel).CornerRadius=UDim.new(0,12)
        local APStroke=Instance.new("UIStroke",APanel)
        APStroke.Thickness=1.5 APStroke.Color=Color3.fromRGB(180,30,60)

        local ATitle=Instance.new("TextLabel",APanel)
        ATitle.Size=UDim2.new(1,0,0,36) ATitle.BackgroundColor3=Color3.fromRGB(180,30,60)
        ATitle.BorderSizePixel=0 ATitle.Text="⚙ ADMIN PANEL"
        ATitle.TextColor3=Color3.fromRGB(255,255,255) ATitle.TextScaled=true
        ATitle.Font=Enum.Font.GothamBold
        Instance.new("UICorner",ATitle).CornerRadius=UDim.new(0,12)

        local AClose=Instance.new("TextButton",APanel)
        AClose.Size=UDim2.new(0,30,0,30) AClose.Position=UDim2.new(1,-34,0,3)
        AClose.BackgroundTransparency=1 AClose.Text="✕" AClose.TextScaled=true
        AClose.TextColor3=Color3.fromRGB(255,255,255) AClose.Font=Enum.Font.GothamBold
        AClose.BorderSizePixel=0
        AClose.MouseButton1Click:Connect(function() APanel.Visible=false APanelOpen=false end)

        local tabs={"FEATURE","LOG","INFO"}
        local tabBtns={} local tabFrames={}
        for i,name in ipairs(tabs) do
            local tb=Instance.new("TextButton",APanel)
            tb.Size=UDim2.new(0,100,0,26) tb.Position=UDim2.new(0,8+(i-1)*108,0,42)
            tb.BackgroundColor3=Color3.fromRGB(40,20,60)
            tb.TextColor3=Color3.fromRGB(200,180,220) tb.Text=name
            tb.TextScaled=true tb.Font=Enum.Font.GothamBold tb.BorderSizePixel=0
            Instance.new("UICorner",tb).CornerRadius=UDim.new(0,6)
            tabBtns[i]=tb
            local tf=Instance.new("Frame",APanel)
            tf.Size=UDim2.new(1,-16,1,-80) tf.Position=UDim2.new(0,8,0,74)
            tf.BackgroundTransparency=1 tf.BorderSizePixel=0 tf.Visible=i==1
            tabFrames[i]=tf
        end
        local function switchTab(idx)
            for i,tf in ipairs(tabFrames) do
                tf.Visible=i==idx
                tabBtns[i].BackgroundColor3=i==idx and Color3.fromRGB(180,30,60) or Color3.fromRGB(40,20,60)
            end
        end
        ...
        scroll1.Size=UDim2.new(1,0,1,0) scroll1.BackgroundTransparency=1
        scroll1.BorderSizePixel=0 scroll1.ScrollBarThickness=4
        scroll1.CanvasSize=UDim2.new(0,0,0,0)
        local uil=Instance.new("UIListLayout",scroll1) uil.Padding=UDim.new(0,4)

        local fList={
            {"Walk Speed","WalkSpeed"},{"Jump Power","JumpEnabled"},
            {"เก็บของใต้ดิน","SitEnabled"},{"Anti Look","AntiLook"},
            {"Ghost Shake","GhostShake"},{"กันตาย","AntiDeath"},
            {"ESP Box","ESPBox"},{"ESP Name","ESPName"},{"ESP Item","ESPItem"},
            {"Silent Aim","SilentAim"},{"Gun Mods","GunMods"},
            {"Vehicle Shake","VehicleShake"},{"Anti Ram","AntiRam"},
            {"FPS Booster","FPSBooster"},{"PigHub Effect","PigHubEffect"},
        }
        for _,f in ipairs(fList) do
            local row=Instance.new("Frame",scroll1)
            row.Size=UDim2.new(1,0,0,30) row.BackgroundColor3=Color3.fromRGB(16,10,28)
            row.BorderSizePixel=0
            Instance.new("UICorner",row).CornerRadius=UDim.new(0,6)
            local lbl=Instance.new("TextLabel",row)
            lbl.Size=UDim2.new(0.65,0,1,0) lbl.Position=UDim2.new(0,8,0,0)
            lbl.BackgroundTransparency=1 lbl.Text=f[1] lbl.TextScaled=true
            lbl.Font=Enum.Font.Gotham lbl.TextColor3=Color3.fromRGB(220,200,240)
            lbl.TextXAlignment=Enum.TextXAlignment.Left
            local tog=Instance.new("TextButton",row)
            tog.Size=UDim2.new(0,60,0,22) tog.Position=UDim2.new(1,-68,0.5,-11)
            tog.BackgroundColor3=Color3.fromRGB(60,200,80) tog.Text="ON"
            tog.TextScaled=true tog.Font=Enum.Font.GothamBold
            tog.TextColor3=Color3.fromRGB(255,255,255) tog.BorderSizePixel=0
            Instance.new("UICorner",tog).CornerRadius=UDim.new(0,6)
            local fkey=f[2]
            tog.MouseButton1Click:Connect(function()
                _adminFeatures[fkey]=not _adminFeatures[fkey]
                local on=_adminFeatures[fkey]
                tog.Text=on and "ON" or "OFF"
                tog.BackgroundColor3=on and Color3.fromRGB(60,200,80) or Color3.fromRGB(200,40,40)
                _CFG[fkey]=on _saveSettings()
                _logAdd((on and "[เปิด] " or "[ปิด] ")..f[1])
            end)
        end
        scroll1.CanvasSize=UDim2.new(0,0,0,#fList*34)

        -- TAB 2: LOG
        local logBox=Instance.new("ScrollingFrame",tabFrames[2])
        logBox.Size=UDim2.new(1,0,1,0) logBox.BackgroundColor3=Color3.fromRGB(6,4,12)
        logBox.BorderSizePixel=0 logBox.ScrollBarThickness=4
        logBox.CanvasSize=UDim2.new(0,0,0,0)
        Instance.new("UICorner",logBox).CornerRadius=UDim.new(0,8)
        local logLbl=Instance.new("TextLabel",logBox)
        logLbl.Size=UDim2.new(1,-8,1,0) logLbl.Position=UDim2.new(0,4,0,0)
        logLbl.BackgroundTransparency=1 logLbl.TextColor3=Color3.fromRGB(160,255,160)
        logLbl.Font=Enum.Font.Code logLbl.TextScaled=false logLbl.TextSize=11
        logLbl.TextXAlignment=Enum.TextXAlignment.Left
        logLbl.TextYAlignment=Enum.TextYAlignment.Top logLbl.TextWrapped=true
        task.spawn(function()
            while AG and AG.Parent do
                if tabFrames[2].Visible then
                    logLbl.Text=#_adminLog>0 and table.concat(_adminLog,"\n") or "-- ไม่มี log --"
                    logBox.CanvasSize=UDim2.new(0,0,0,math.max(#_adminLog*14,logBox.AbsoluteSize.Y))
                end
                task.wait(1)
            end
        end)
        ...
        -- TAB 3: INFO
        local infoLbl=Instance.new("TextLabel",tabFrames[3])
        infoLbl.Size=UDim2.new(1,0,1,0) infoLbl.BackgroundTransparency=1
        infoLbl.TextColor3=Color3.fromRGB(200,180,255) infoLbl.Font=Enum.Font.Code
        infoLbl.TextScaled=false infoLbl.TextSize=13
        ...
        ABtn.MouseButton1Click:Connect(function()
            APanelOpen=not APanelOpen
            APanel.Visible=APanelOpen
            if APanelOpen then switchTab(1) end
        end)
        ...
        task.spawn(function()
            ...
            while AG and AG.Parent do
                t=t+0.01
                APStroke.Color=Color3.fromHSV(t%1,0.8,1)
                task.wait(0.05)
            end
        end)

        _logAdd("Admin panel โหลดสำเร็จ")
    end)
end
...
local _SETTINGS_FILE = "phsettings.json"
local _CFG = {}

local function _saveSettings()
    pcall(function()
        writefile(_SETTINGS_FILE, game:GetService("HttpService"):JSONEncode(_CFG))
    end)
end

local function _loadSettings()
    local ok, raw = pcall(readfile, _SETTINGS_FILE)
    if ok and raw and raw ~= "" then
        local jok, d = pcall(function() return game:GetService("HttpService"):JSONDecode(raw) end)
        if jok and type(d) == "table" then _CFG = d end
    end
end

local function _cfg(key, default)
    if _CFG[key] ~= nil then return _CFG[key] end
    return default
end
...
PlayerTab:Toggle({Title = "Walk Speed", Default = _cfg("WalkSpeed", false), Callback = function(state) walkSpeedEnabled = state _CFG["WalkSpeed"] = state _saveSettings() end})
PlayerTab:Slider({Title = "Speed Value", Step = 0.01, Value = {Min = 0.01, Max = 0.3, Default = _cfg("SpeedValue", 0.05)}, Callback = function(v) speedValue = v _CFG["SpeedValue"] = v _saveSettings() end})
...
PlayerTab:Toggle({
    ...
    Default = _cfg("InfStamina", false),
    Callback = function(state)
        _CFG["InfStamina"] = state _saveSettings()
        if state then
...
PlayerTab:Toggle({Title = "Jump Power", Default = _cfg("JumpEnabled", false), Callback = function(state)
    jumpEnabled = state _CFG["JumpEnabled"] = state _saveSettings()
...
PlayerTab:Slider({Title = "Jump Height", Step = 5, Value = {Min = 20, Max = 80, Default = _cfg("JumpPower", 70)}, Callback = function(v) jumpPower = v _CFG["JumpPower"] = v _saveSettings() end})

PlayerTab:Divider()
PlayerTab:Section({Title = "SIT SYSTEM"})
PlayerTab:Toggle({Title = "เก็บของใต้ดิน", Default = _cfg("SitEnabled", false), Callback = function(state) toggleSitSystem(state) _CFG["SitEnabled"] = state _saveSettings() end})
PlayerTab:Slider({Title = "ปรับ Height", Step = 0.1, Value = {Min = -5, Max = 4, Default = _cfg("SitHeight", 0)}, Callback = function(v) sitHeight = v _CFG["SitHeight"] = v _saveSettings() end})

PlayerTab:Divider()
PlayerTab:Section({Title = "ANTI LOOK"})
PlayerTab:Toggle({Title = "Anti Look", Default = _cfg("AntiLook", false), Callback = function(state) toggleAntiLook(state) _CFG["AntiLook"] = state _saveSettings() end})
PlayerTab:Slider({Title = "Strength", Step = 100, Value = {Min = 500, Max = 3500, Default = _cfg("AntiLookStrength", 3500)}, Callback = function(v) getgenv().SkyAmount = v _CFG["AntiLookStrength"] = v _saveSettings() end})
...
PlayerTab:Toggle({Title = "Ghost Shake (คนอื่นเห็นสั่น)", Default = _cfg("GhostShake", false), Callback = function(state)
    ghostShakeEnabled = state _CFG["GhostShake"] = state _saveSettings()
...
PlayerTab:Toggle({Title = "กันตาย", Default = _cfg("AntiDeath", false), Callback = function(state)
    antiDeathEnabled = state _CFG["AntiDeath"] = state _saveSettings()
...
VehicleTab:Slider({Title = "Pull All Radius", Step = 5, Value = {Min = 10, Max = 100, Default = _cfg("PullRadius", 30)}, Callback = function(v) pullAllRadius = v _CFG["PullRadius"] = v _saveSettings() end})
...
VehicleTab:Toggle({Title = "สั่นรถทุกคันในเกม", Default = _cfg("VehicleShake", false), Callback = function(state)
    vehicleShakeEnabled = state _CFG["VehicleShake"] = state _saveSettings()
...
VehicleTab:Slider({Title = "Vehicle Speed Boost", Step = 5, Value = {Min = 0, Max = 200, Default = _cfg("VehicleSpeed", 50)}, Callback = function(v) vehicleSpeedBoost = v _CFG["VehicleSpeed"] = v _saveSettings() end})
VehicleTab:Toggle({Title = "Enable Speed Boost", Default = _cfg("VehicleSpeedOn", false), Callback = function(state)
    _CFG["VehicleSpeedOn"] = state _saveSettings()
    if state then
        local speedConn
...
VehicleTab:Slider({Title = "Anti Ram Distance", Step = 5, Value = {Min = 20, Max = 80, Default = _cfg("AntiRamDist", 45)}, Callback = function(v) antiRamDistance = v _CFG["AntiRamDist"] = v _saveSettings() end})
VehicleTab:Toggle({Title = "Enable Anti Ram", Default = _cfg("AntiRamOn", false), Callback = function(state)
    antiRamEnabled = state _CFG["AntiRamOn"] = state _saveSettings()
    if state then
        local antiRamConn
...
ESPTab:Toggle({Title = "ESP Box", Default = _cfg("ESPBox", false), Callback = function(v)
    ShowBoxESP = v _CFG["ESPBox"] = v _saveSettings()
...
ESPTab:Dropdown({Title = "Box Color", Values = {"Rainbow","Green","White","Red","Blue","Yellow"}, Default = _cfg("BoxColor", 1), Callback = function(v) BOX_COLOR_MODE = v:lower() _CFG["BoxColor"] = v _saveSettings() end})
