...
-- ========== AUTO START CONFIG ==========
-- เปิดใช้งานอัตโนมัติโดยไม่ต้องกดคีย์ลัด
getgenv().AutoFarm_Level = true  -- เปิด Auto Farm Level
getgenv().AutoFarmNearest = false -- ปิด Auto Farm Nearest (จะใช้แค่ Level)
getgenv().FastAttack = true      -- เปิด Fast Attack
getgenv().SWITCH_TARGET = true   -- เปิดสลับเป้าหมาย
getgenv().StayOnHead = true      -- เปิดระบบอยู่บนหัวมอนสเตอร์
...
-- ========== ระบบอยู่บนหัวมอนสเตอร์ ==========
local CurrentEnemy = nil

local function GetHeadPosition(enemy)
    -- พยายามหา Head ก่อน
    local head = enemy:FindFirstChild("Head")
    if head and head:IsA("BasePart") then
        return head.CFrame + Vector3.new(0, 2, 0)
    end
    -- ถ้าไม่มี Head ให้ใช้ HumanoidRootPart + สูงขึ้นไป
    local hrp = enemy:FindFirstChild("HumanoidRootPart")
    if hrp and hrp:IsA("BasePart") then
        return hrp.CFrame + Vector3.new(0, 4, 0)
    end
    return nil
end

-- ทำให้ตัวละครอยู่บนหัวมอนสเตอร์แบบ Real-time
task.spawn(function()
    while task.wait() do
        pcall(function()
            if getgenv().AutoFarm_Level and getgenv().StayOnHead and CurrentEnemy and CurrentEnemy.Parent then
                local char = Player.Character
                if not char then return end
                
                local hrp = char:FindFirstChild("HumanoidRootPart")
                local headPos = GetHeadPosition(CurrentEnemy)
                
                if hrp and headPos then
                    -- ตั้ง CFrame ให้อยู่บนหัวมอนสเตอร์โดยตรง
                    hrp.CFrame = headPos
                    
                    -- ปิดการชนของทุกส่วนในตัวละคร เพื่อไม่ให้โดนผลักตก
                    ...
                    -- ทำให้มอนสเตอร์อยู่กับที่ ไม่เคลื่อนที่
                    local humanoid = CurrentEnemy:FindFirstChild("Humanoid")
                    if humanoid then
                        humanoid.WalkSpeed = 0
                        humanoid.JumpPower = 0
                        -- ทำให้มอนสเตอร์อยู่ในสถานะล้ม/นิ่ง
                        pcall(function()
...
local function AutoFarm_Level()
  while getgenv().AutoFarm_Level do task.wait()
    ...
    -- อัพเดท CurrentEnemy สำหรับระบบอยู่บนหัว
    if Enemie and Enemie.Parent then
      CurrentEnemy = Enemie
    end
    ...
    elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
      -- ถ้าเปิดระบบอยู่บนหัว จะไม่ใช้ FarmPos ปกติ
      if not getgenv().StayOnHead then
...
UIS.InputBegan:Connect(function(i, g)
  ...
  elseif i.KeyCode == Enum.KeyCode.RightShift then
    getgenv().StayOnHead = not getgenv().StayOnHead
    SG:SetCore("SendNotification", {Title="Stay On Head", Text=getgenv().StayOnHead and "ON" (อยู่บนหัวมอน)" or "OFF", Duration=1})
  end
end)
...
game:GetService("StarterGui"):SetCore("SendNotification", {
  ...
  Text = "AutoFarm เปิดอัตโนมัติ! | F1=ปิด/เปิด | RCtrl=FastAttack | RShift=อยู่บนหัวมอน",
  Duration = 5
})