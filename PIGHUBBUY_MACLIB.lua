...
local _E1 = {66,94,94,90,89,16,5,5,88,75,93,4,77,67,94,66,95,72,95,89,79,88,73,69,68,94,79,68,94,4,73,69,71,5,65,65,79,66,69,78,7,71,75,82,5,7,30,24,24,30,31,28,29,18,5,88,79,76,89,5,66,79,75,78,89,5,71,75,67,68,5,65,65,79,66,69,78,4,64,89,69,68}
...
task.spawn(function()
    ...
    if _ktype == "admin" or _ktype == "adminmax" then
...
if _ktype == "adminmax" then
    ...
    AdminTab:Button({Title = "แสดง HWID", Callback = function()
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "HWID", Text = _getHWID(), Duration = 5})
    end})
    AdminTab:Button({Title = "รีเซ็ตคีย์", Callback = function()
        pcall(function() writefile("phs.txt", "") end)
        game:GetService("Players").LocalPlayer:Kick("คีย์ถูกรีเซ็ต กรุณาใส่คีย์ใหม่")
    end})
    AdminTab:Button({Title = "ตรวจสอบสถานะคีย์", Callback = function()
        local s = _load()
        if s and s.key then
            local expText = s.expires and os.date("%Y-%m-%d %H:%M:%S", s.expires) or "ถาวร"
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", {Title = "สถานะคีย์", Text = "ไม่พบคีย์", Duration = 3})
        end
    end})
    AdminTab:Button({Title = "ดึงข้อมูลคีย์จากเซิร์ฟเวอร์", Callback = function()
        local data = _fetch()
        if data then
            local count = 0
    ...
    AdminTab:Button({Title = "แสดงรายชื่อผู้เล่น", Callback = function()
        local players = {}
        for _, p in ipairs(Players:GetPlayers()) do table.insert(players, p.Name.." ("..p.UserId..")") end
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "ผู้เล่น", Text = table.concat(players, ", "), Duration = 10})
    end})
    ...
    AdminTab:Input({Title = "รายชื่อ Admin", Placeholder = "ชื่อผู้เล่น...", Callback = function(v)
        adminPlayers = {}
        for name in string.gmatch(v, "%S+") do adminPlayers[name] = true end
    end})
    AdminTab:Button({Title = "รีเซ็ตค่าทั้งหมด", Callback = function()
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "รีเซ็ตสำเร็จ", Text = "ค่าทั้งหมดกลับสู่ค่าเริ่มต้น", Duration = 3})
    end})
end

-- ========== PLAYER TAB ==========
PlayerTab:Section({Title = "สถิติผู้เล่น"})
local BankBalance = PlayerTab:Button({Title = "เงินในธนาคาร", Desc = "$0", Callback = function() end})
local HandBalance = PlayerTab:Button({Title = "เงินสด", Desc = "$0", Callback = function() end})
task.spawn(function()
...
PlayerTab:Slider({Title = "ความเร็ว", Step = 0.01, Value = {Min = 0.01, Max = 0.3, Default = 0.05}, Callback = function(v) speedValue = v end})
...
PlayerTab:Toggle({Title = "นั่งใต้ดิน (คงที่)", Default = false, Callback = function(state) toggleSitSystem(state) end})
...
PlayerTab:Toggle({Title = "ป้องกันการมอง", Default = false, Callback = function(state) toggleAntiLook(state) end})
PlayerTab:Slider({Title = "ความแรง", Step = 100, Value = {Min = 500, Max = 3500, Default = 3500}, Callback = function(v) getgenv().SkyAmount = v end})

PlayerTab:Toggle({Title = "สั่นไหว (คนอื่นเห็น)", Default = false, Callback = function(state)
...
VehicleTab:Button({Title = "ดึงรถของฉัน", Callback = function()
    ...
    if #myVehicles == 0 then
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "รถ", Text = "ไม่พบรถของคุณ", Duration = 3}) return
    end
    local pulled = 0
...
VehicleTab:Slider({Title = "รัศมีดึงรถคนอื่น", Step = 5, Value = {Min = 10, Max = 100, Default = 30}, Callback = function(v) pullAllRadius = v end})
...
VehicleTab:Button({Title = "ซ่อมรถที่ขี่", Callback = function()
    ...
    local seat = char:FindFirstChild("SeatPart") or char:FindFirstChild("SeatWeld")
    if not seat then
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "รถ", Text = "คุณไม่ได้นั่งรถ", Duration = 3}) return
    end
    local vehicle = seat.Parent
    while vehicle and not vehicle:IsA("Model") do vehicle = vehicle.Parent end
    if vehicle then
