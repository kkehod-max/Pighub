...
-- ระบบป้องกันการตาย (Anti Death) มาไว้ก่อนพวก Silent Aim
...
RunService.RenderStepped:Connect(function()
    ...
    for i = 1, 8 do
        ...
        FOV_Lines[i].Color = Color3.fromRGB(100, 255, 120)
    ...
    if CurrentTarget and CurrentTarget.Character then
        ...
        if hum and hum.Health > 0 and targetPart then
            ...
            if onScreen then
                ...
                SA_Tracer.To = tPos
                SA_Tracer.Color = Color3.fromRGB(0, 180, 30)
                ...
                for i = 1, 8 do
                    ...
                    Target_Lines[i].Color = Color3.fromRGB(80, 255, 100)
        ...
        if SilentAimEnabled and args[2] == "shoot_gun" and CurrentTarget then
            ...
            if targetPart and hum and hum.Health > 0 then
                ...
                if myPos then
                    ...
                    local aimPos = predictPosition(targetPart, char:FindFirstChild("HumanoidRootPart"))
                    if isShotgun() then
                    ...
                    task.spawn(function()
                        local distance = (myPos - aimPos).Magnitude
                        if distance < 1 then return end
                        local beam = Instance.new("Part")
                        beam.Anchored = true
                        beam.CanCollide = false
                        beam.Size = Vector3.new(0.15, 0.15, distance)
                        beam.CFrame = CFrame.new(myPos, aimPos) * CFrame.new(0, 0, -distance/2)
                        beam.Material = Enum.Material.Neon
                        beam.Transparency = 0.25
                        beam.Color = Color3.fromHSV((_shotCounter * 0.08) % 1, 1, 1)
    ...
    for _, item in ipairs(dropped2:GetChildren()) do
        if item:IsA("Model") and item:FindFirstChild("PickUpZone") and not item:GetAttribute("Locked") then
            ...
            if ok2 and pos2 then
                ...
                if dist2 > 500 then continue end
                ...
                if onScreen2 then
                    ...
                    if item.Name == "Money" then color2 = Color3.fromRGB(0, 255, 0) end
...
PlayerTab:Toggle({Title = "ไม่มีขีดจำกัดพลังวิ่ง", Default = false, Callback = function(state) toggleInfStamina(state) end})
