...
CampName.TextColor3 = Color3.fromRGB(138, 43, 226)
...
local function _showKey(cb)
    ...
    makeAuraLayer(0.55, Color3.fromRGB(100,0,180), Color3.fromRGB(0,0,0), 0.8, 0)
    makeAuraLayer(0.42, Color3.fromRGB(140,0,220), Color3.fromRGB(50,0,110), 1.1, 0.1)
    makeAuraLayer(0.30, Color3.fromRGB(150,50,255), Color3.fromRGB(80,0,180), 1.4, 0.05)
    makeAuraLayer(0.18, Color3.fromRGB(200,140,255), Color3.fromRGB(130,0,230), 1.8, 0.15)
    task.spawn(function()
        for _ = 1, 30 do
    ...
    CardStroke.Color = Color3.fromRGB(120,40,220)
    local TopBar = Instance.new("Frame", Card)
    TopBar.Size = UDim2.new(1,0,0,3)
    TopBar.Position = UDim2.new(0,0,0,0)
    TopBar.BorderSizePixel = 0
    TopBar.BackgroundColor3 = Color3.fromRGB(120,40,220)
    Instance.new("UICorner", TopBar).CornerRadius = UDim.new(0,18)
    local TopGrad = Instance.new("UIGradient", TopBar)
    TopGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(150,50,255)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(100,30,210)),
    ...
    Sub.TextScaled = true
    Sub.TextColor3 = Color3.fromRGB(130,100,180)
    ...
    IS.Thickness = 1
    IS.Color = Color3.fromRGB(90,40,180)
    ...
    IB.PlaceholderColor3 = Color3.fromRGB(90,60,140)
    ...
    BtnGrad.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(1, Color3.fromRGB(80,20,190)),
    })
    BtnGrad.Rotation = 90
...
task.spawn(function()
    ...
    KF.BackgroundColor3 = Color3.fromRGB(8,5,20)
    ...
    KL.Size = UDim2.new(1,-52,1,0) KL.Position = UDim2.new(0,4,0,0)
    LogoutBtn.MouseButton1Click:Connect(function()
        pcall(function() writefile("phs.txt", "") end)
        KG:Destroy()
        _showKey(function(t, e) _ktype = t _kexp = e end)
    end)
        ...
        LogoutBtn.MouseButton1Click:Connect(function()
            ...
            _showKey(function(t, e)
                ...
                task.spawn(function()
                    ...
                    KF2.BackgroundColor3 = Color3.fromRGB(8,5,20)
...
-- [[ SETTINGS SAVE/LOAD ]] --
local _SF = "phsettings.json"
local _CFG = {}
local function _saveCFG()
    pcall(function() writefile(_SF, game:GetService("HttpService"):JSONEncode(_CFG)) end)
end
local function _loadCFG()
    local ok,v = pcall(readfile, _SF)
    if ok and v and v~="" then
        local jok,d = pcall(function() return game:GetService("HttpService"):JSONDecode(v) end)
        if jok and type(d)=="table" then _CFG=d end
    end
end
local function _cfg(k, def)
    return _CFG[k]~=nil and _CFG[k] or def
end
_loadCFG()
...
RunService.Heartbeat:Connect(function()
    local now = os.clock()
    if now - _lastHistoryUpdate < 0.016 then return end
...
-- ดึง velocity รถ (จาก WeldConstraint/Weld)
local function getVehicleVelocity(char)
    if not char then return nil end
    local root = char:FindFirstChild("HumanoidRootPart")
    if not root then return nil end
    -- วิธีที่ 1: ดู AssemblyLinearVelocity ของ parts ที่ connect อยู่
    local ok, vel = pcall(function()
        for _, v in ipairs(root:GetConnectedParts(true)) do
            if v:IsA("BasePart") and v.Parent ~= char then
                local v2 = v.AssemblyLinearVelocity
                if v2.Magnitude > 8 then return v2 end
            end
        end
        return nil
    end)
    if ok and vel then return vel end
    -- วิธีที่ 2: ดู Weld/WeldConstraint (แบบ reference script)
    local seat = root:FindFirstChildWhichIsA("WeldConstraint") or root:FindFirstChildWhichIsA("Weld")
    if seat then
        local part = (seat:IsA("WeldConstraint") and seat.Part0) or (seat:IsA("Weld") and seat.Part0)
        if part and part:IsA("BasePart") then
            local v = part.AssemblyLinearVelocity
            if v.Magnitude > 8 then return v end
        end
    end
    return nil
end
...
local function predictPosition(targetPart, hrp)
    ...
    -- เช็ครถก่อน (vehicle velocity แม่นกว่า history)
    local vVel = getVehicleVelocity(char)
    if vVel then
        return targetPart.Position + (vVel * ping * PREDICT_FACTOR * 1.2)
    end

    local velocity = calculateVelocity(player) or Vector3.zero
    local speed = velocity.Magnitude
    local factor = PREDICT_FACTOR
    if speed > 20 then factor = PREDICT_FACTOR * 1.2 end
    if speed > 40 then factor = PREDICT_FACTOR * 1.5 end

    return targetPart.Position + (velocity * ping * factor)
end
...
pcall(function()
    _oldFire = hookfunction(_SA_Remote.FireServer, function(self, ...)
        ...
        elseif args[2] == "shoot_gun" then
            -- ยิงปกติ (ไม่มีเป้า) ก็แสดง tracer ตาม CFrame
            local myHead = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head")
...
local BankBalance = PlayerTab:Button({Title = "เงินในธนาคาร", Desc = "<b><font color='#9B30FF'>$0</font></b>", Callback = function() end})
local HandBalance = PlayerTab:Button({Title = "เงินสด", Desc = "<b><font color='#C080FF'>$0</font></b>", Callback = function() end})
...
PlayerTab:Toggle({Title = "เดินเร็ว", Default = _cfg("WalkSpeed",false), Callback = function(state) walkSpeedEnabled=state _CFG["WalkSpeed"]=state _saveCFG() end})
PlayerTab:Slider({Title = "ความเร็ว", Step = 0.01, Value = {Min = 0.01, Max = 0.25, Default = _cfg("SpeedVal",0.05)}, Callback = function(v) speedValue=v _CFG["SpeedVal"]=v _saveCFG() end})
...
PlayerTab:Slider({Title = "ความสูงกระโดด", Step = 5, Value = {Min = 20, Max = 80, Default = _cfg("JumpH",70)}, Callback = function(v) jumpPower=v _CFG["JumpH"]=v _saveCFG() end})
...
PlayerTab:Slider({Title = "ความลึก", Step = 0.1, Value = {Min = -2.5, Max = -2, Default = _cfg("SitH",-2)}, Callback = function(v) sitHeight=v _CFG["SitH"]=v _saveCFG() end})
...
VehicleTab:Slider({Title = "ความเร็ว", Step = 5, Value = {Min = 0, Max = 200, Default = _cfg("VehSpeed",50)}, Callback = function(v) vehicleSpeedBoost=v _CFG["VehSpeed"]=v _saveCFG() end})
...
ESPTab:Toggle({Title = "แสดงชื่อ", Default = _cfg("ESPName",false), Callback = function(v) ShowNameESP=v _CFG["ESPName"]=v _saveCFG() end})
ESPTab:Slider({Title = "ขนาดตัวอักษร", Step = 1, Value = {Min = 12, Max = 28, Default = _cfg("NameSize",18)}, Callback = function(v) nameFontSize=v _CFG["NameSize"]=v _saveCFG() end})
...
ESPTab:Toggle({Title = "แสดงไอเทมบนพื้น", Default = _cfg("ItemESP",false), Callback = function(v) getgenv().ItemESPEnabled=v _CFG["ItemESP"]=v _saveCFG() end})
...
PVPTab:Slider({Title = "รัศมี FOV", Step = 10, Value = {Min = 50, Max = 600, Default = _cfg("FOV",200)}, Callback = function(v) FOVRadius=v _CFG["FOV"]=v _saveCFG() end})
...
GunModTab:Divider()
GunModTab:Slider({Title = "อัตรายิง", Step = 10, Value = {Min = 100, Max = 3000, Default = _cfg("FireRate",1000)}, Callback = function(v) getgenv().FireRateValue=v _CFG["FireRate"]=v _saveCFG() end})
