        ...
        if SilentAimEnabled and args[2] == "shoot_gun" and CurrentTarget then
            ...
            if targetPart and hum and hum.Health > 0 then
                ...
                if myPos then
                    ...
                    local finalDir
                    if _shotCounter % 4 == 0 then
                        finalDir = calculateWaveCurve(myPos, targetPos, _shotCounter)
                    else
                        finalDir = calculateGradualCurve(myPos, targetPos, distance)
                    end
                    local endPoint = myPos + (finalDir * 500)
                    args[4] = CFrame.new(myPos, endPoint)
                    local raycastParams = RaycastParams.new()
                    raycastParams.FilterDescendantsInstances = {LocalPlayer.Character}
                    raycastParams.FilterType = Enum.RaycastFilterType.Blacklist
                    local biasToHead = 1 - (CurveIntensity * 0.3)
                    local finalTargetPos = targetPos * biasToHead + endPoint * (1 - biasToHead)
                    local raycastResult = workspace:Raycast(myPos, (finalTargetPos - myPos).Unit * 500, raycastParams)
                    if raycastResult then
                        args[5] = {{[1]={Instance=targetPart, Normal=raycastResult.Normal, Position=raycastResult.Position}}}
                    else
                        args[5] = {{[1]={Instance=targetPart, Normal=Vector3.new(0,1,0), Position=targetPos}}}
                    end
                    task.spawn(function() CreateBulletBeam(myPos, targetPos) end)
                end
            end
        end
...
PlayerTab:Toggle({Title = "ป้องกันการตาย", Default = false, Callback = function(state) antiDeathEnabled = state if not state then exitSafeMode() end end})
...
ESPTab:Divider()
ESPTab:Section({Title = "ดูดไอเทม"})
ESPTab:Toggle({Title = "เปิดดูดไอเทม", Default = false, Callback = function(v) MagnetPickupEnabled = v if v then startMagnetPickup() else stopMagnetPickup() end end})
ESPTab:Slider({Title = "รัศมีการดูด", Step = 100, Value = {Min = 500, Max = 8000, Default = 2000}, Callback = function(v) MAGNET_RADIUS = v end})
ESPTab:Slider({Title = "ความเร็วดูด", Step = 0.1, Value = {Min = 0.3, Max = 2, Default = 0.8}, Callback = function(v) MAGNET_SPEED = v end})
