...
-- URL 1 (ลูกค้า) + URL 2 (admin) เข้ารหัส XOR
...
-- admin key hardcode แยกจาก JSON
...
-- ========== ADMIN MAX TAB ==========
if _ktype == "adminmax" then
    local AdminMaxTab = Window:Tab({Title = "ADMIN MAX", Icon = "crown"})

    AdminMaxTab:Section({Title = "🔐 ADMIN CONTROL"})

    AdminMaxTab:Button({Title = "📋 แสดง HWID ปัจจุบัน", Callback = function()
    ...
    AdminMaxTab:Button({Title = "🔄 รีเซ็ตคีย์ (logout)", Callback = function()
        pcall(function() writefile("phs.txt", "") end)
        game:GetService("Players").LocalPlayer:Kick("คีย์ถูกรีเซ็ต กรุณาใส่คีย์ใหม่")
    end})

    AdminMaxTab:Button({Title = "🔍 ตรวจสอบสถานะคีย์", Callback = function()
        local s = _load()
        if s and s.key then
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Key Status",
                Text = "ไม่พบคีย์ที่บันทึก",
                Duration = 3
            })
        end
    end})

    AdminMaxTab:Button({Title = "🌐 ดึงข้อมูลคีย์ล่าสุดจากเซิร์ฟเวอร์", Callback = function()
        local data = _fetch()
        if data then
            local count = 0
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Data Refresh",
                Text = "ดึงข้อมูลสำเร็จ (" .. count .. " คีย์)",
                Duration = 5
            })
        else
            game:GetService("StarterGui"):SetCore("SendNotification", {
    ...
    AdminMaxTab:Divider()
    AdminMaxTab:Section({Title = "⚙️ UTILITIES"})

    AdminMaxTab:Button({Title = "📊 แสดงข้อมูลผู้เล่นทั้งหมด", Callback = function()
        local players = {}
    ...
    AdminMaxTab:Button({Title = "🚀 รีเซ็ตทุกอย่าง (กลับสู่หน้าคีย์)", Callback = function()
        pcall(function() writefile("phs.txt", "") end)
        game:GetService("Players").LocalPlayer:Kick("รีเซ็ตระบบ กรุณาเปิดสคริปต์ใหม่")
    end})
end
