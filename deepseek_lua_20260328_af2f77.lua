...
getgenv().FarmPos = getgenv().FarmPos or Vector3.new(0, 15, 0)  -- บินเหนือหัว 15 สตั๊ด
...
getgenv().AutoClickDelay = getgenv().AutoClickDelay or 0.08  -- ลดเวลาคลิก
...
getgenv().MULTI_HIT = getgenv().MULTI_HIT or 25  -- เพิ่ม Multi Hit
getgenv().ATTACK_DELAY = getgenv().ATTACK_DELAY or 0.005  -- ลดดีเลย์ลงมาก
getgenv().DISTANCE = getgenv().DISTANCE or 300

-- ========== OPTIMIZATION - ลดความแล็ค ==========
-- ปรับแต่ง Physics และ Rendering
game:GetService("Workspace").StreamingEnabled = true
game:GetService("Workspace").GlobalPhysicsStepping = 0.03
settings().Rendering.QualityLevel = 1
settings().Rendering.EffectsQuality = 1
settings().Physics.PhysicsEnvironmentalThrottle = Enum.EnviromentalPhysicsThrottle.Disabled

-- ปิด Particle Effects และ Decals
...
-- ========== PLATFORM SYSTEM (ปรับปรุงให้บินเหนือหัว) ==========
    ...
    pcall(function()
      if getgenv().AutoFarm_Level then
        ...
        if plrChar then
          ...
          -- ทำให้บินอยู่เหนือหัวมอนสเตอร์เสมอ
...
local function AutoFarm_Level()
  while getgenv().AutoFarm_Level do task.wait()
    ...
    elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
      -- บินเหนือหัวมอนสเตอร์โดยตรง
      local flyCFrame = Enemie.HumanoidRootPart.CFrame + getgenv().FarmPos
      PlayerTP(flyCFrame)
      pcall(function() 
        PlayerClick() 
        ActiveHaki() 
...
local function FA_DoAttack(HRP)
  ...
  pcall(function()
    ...
    -- ส่ง Hit หลายๆครั้งแบบรวดเร็วโดยไม่ใช้ task.wait มากเกินไป
...
game:GetService("StarterGui"):SetCore("SendNotification", {
  ...
  Text = "บินเหนือหัวมอนสเตอร์ | Fast Attack เร็วขึ้น | ลดแล็คเรียบร้อย!",
  Duration = 5
})