...
CampName.TextColor3 = Color3.fromRGB(128, 0, 255)
...
SubText.TextColor3 = Color3.fromRGB(180, 130, 255)
...
local function _showKey(cb)
    ...
    makeAuraLayer(0.55, Color3.fromRGB(75,0,130), Color3.fromRGB(0,0,0), 0.8, 0)
    makeAuraLayer(0.42, Color3.fromRGB(128,0,255), Color3.fromRGB(45,0,90), 1.1, 0.1)
    makeAuraLayer(0.30, Color3.fromRGB(180,80,255), Color3.fromRGB(75,0,130), 1.4, 0.05)
    makeAuraLayer(0.18, Color3.fromRGB(220,160,255), Color3.fromRGB(128,0,255), 1.8, 0.15)
    task.spawn(function()
        for _ = 1, 30 do
            ...
            dot.BackgroundColor3 = Color3.fromHSV(0.75, 1, math.random(70,100)*0.01)
    ...
    Card.BackgroundColor3 = Color3.fromRGB(10,5,20)
    Card.BorderSizePixel = 0
    Instance.new("UICorner", Card).CornerRadius = UDim.new(0,18)
    local CardStroke = Instance.new("UIStroke", Card)
    CardStroke.Thickness = 1.5
    CardStroke.Color = Color3.fromRGB(128,0,255)
    local TopBar = Instance.new("Frame", Card)
    TopBar.Size = UDim2.new(1,0,0,3)
    TopBar.Position = UDim2.new(0,0,0,0)
    TopBar.BorderSizePixel = 0
    TopBar.BackgroundColor3 = Color3.fromRGB(128,0,255)
    ...
    task.spawn(function()
        ...
        while G and G.Parent do
            t = t + 0.012
            CardStroke.Color = Color3.fromHSV(0.75, 0.85, 1)
            IS.Color = Color3.fromHSV(0.7, 0.6, 0.9)
    ...
    Btn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            SL.Text = _typeLabel(ktype) SL.TextColor3 = Color3.fromRGB(150,100,255)
            task.spawn(function()
...
-- ============================================
-- ระบบ Config Auto Save/Load
-- ============================================
local CONFIG_FILE = "pighub_config.json"
local Config = {
    ...
    shakeStrength = 2500,
    randomPatternEnabled = false,
    antiDeathEnabled = false,
    SilentAimEnabled = false,
    FOVRadius = 200,
    SelectedAimPart = "Head",
    excludedPlayerNames = {},
    safeAdminEnabled = false,
    adminPlayers = {},
    GunModsAutoApply = false,
    FireRateValue = 1000,
    AccuracyValue = 1,
    RecoilValue = 0,
    DurabilityValue = 999999999,
    AutoValue = true,
    FistsBuffEnabled = false,
    ShowBoxESP = false,
    BOX_COLOR_MODE = "rainbow",
    ShowNameESP = false,
    nameFontSize = 18,
    ShowItemESP = false,
    itemBillboardSize = 26,
    ItemESPEnabled = false,
    ItemESPMaxDist = 500,
    AutoSkipEnabled = false,
    vehicleSpeedBoost = 50,
    antiRamEnabled = false,
    antiRamDistance = 45,
    vehicleShakeEnabled = false,
    fpsBoosterEnabled = false,
    fpsBoosterMode = 1,
    PigHubEffectEnabled = false,
    ORBIT_RADIUS = 5,
    ORBIT_SPEED = 2.5
}
...
local function SaveConfig()
    pcall(function()
        writefile(CONFIG_FILE, _HS:JSONEncode(Config))
    end)
end
...
local function LoadConfig()
    ...
    if ok and data and data ~= "" then
        local success, decoded = pcall(_HS.JSONDecode, _HS, data)
...
-- ============================================
-- ระบบ Switch Account สำหรับทุกคน + แสดงสถานะคีย์
-- ============================================
...
local function SwitchAccount()
    -- ลบ GUI สถานะเดิม
    pcall(function()
        local oldKG = CoreGui:FindFirstChild("PHKInfo")
        if oldKG then oldKG:Destroy() end
    end)
...
-- ฟังก์ชันสร้าง GUI สถานะคีย์ (แยกไว้ให้เรียกซ้ำได้)
function CreateKeyStatusGUI()
    -- ลบของเก่าถ้ามี
    pcall(function()
        local oldKG = CoreGui:FindFirstChild("PHKInfo")
        if oldKG then oldKG:Destroy() end
    ...
    KF.Size = UDim2.new(0,300,0,28)
    KF.Position = UDim2.new(0,8,0,8)
    KF.BackgroundColor3 = Color3.fromRGB(15,10,25)
    ...
    -- ปุ่ม Switch Account (สำหรับทุกคน)
    local SwitchBtn = Instance.new("TextButton", KF)
    SwitchBtn.Size = UDim2.new(0,90,1,0)
    SwitchBtn.Position = UDim2.new(1,-94,0,0)
    SwitchBtn.BackgroundColor3 = Color3.fromRGB(128,0,80)
    ...
    -- อัปเดตสถานะคีย์แบบเรียลไทม์
    task.spawn(function()
        local lastRem = nil
        while KG and KG.Parent do
            task.wait(1)
            KS.Color = Color3.fromHSV(0.75,0.6,1)
            if _ktype == "adminmax" or _ktype == "admin" then
                if _kexp == nil then
                    KL.Text = _typeLabel(_ktype).." | ∞"
                    KL.TextColor3 = Color3.fromRGB(150,100,255)
                else
                    ...
                    KL.TextColor3 = rem < 3600 and Color3.fromRGB(255,150,50) or Color3.fromRGB(150,100,255)
                end
            else
                if _kexp == nil then
                    KL.Text = _typeLabel(_ktype).." | ∞"
...
-- ============================================
-- ส่วนที่เหลือของสคริปต์ (ระบบต่างๆ)
-- ============================================
...
pcall(function()
    if Window and Window.UI and Window.UI.Background then
        Window.UI.Background.BackgroundColor3 = Color3.fromRGB(8, 4, 16)
        Window.UI.Background.BackgroundTransparency = 0
    end
    if Window and Window.SideBar then
        Window.SideBar.BackgroundColor3 = Color3.fromRGB(10, 5, 20)
    end
end)
...
local jumpEnabled = Config.jumpEnabled
local jumpPower = Config.jumpPower
local jumpConnection = nil

local sitEnabled = Config.sitEnabled
local sitHeight = Config.sitHeight
local sitConnection = nil

local function toggleSitSystem(state)
    sitEnabled = state
    Config.sitEnabled = state
    SaveConfig()
...
local speedValue = Config.speedValue
...
-- ANTI LOOK SYSTEM
-- ============================================
local antiLookEnabled = Config.antiLookEnabled
local shakeStrength = Config.shakeStrength
local twistEnabled = Config.antiLookEnabled
local randomPatternEnabled = Config.randomPatternEnabled
local shakeConnection = nil
local twistConnection = nil
local ghostShakeEnabled = Config.antiLookEnabled
...
local function toggleRandomPattern(state)
    ...
    Config.randomPatternEnabled = state
    SaveConfig()
...
-- ANTI DEATH
-- ============================================
local antiDeathEnabled = Config.antiDeathEnabled
...
local SilentAimEnabled = Config.SilentAimEnabled
local FOVRadius = Config.FOVRadius
local CurrentTarget = nil
local SelectedAimPart = Config.SelectedAimPart
local excludedPlayerNames = Config.excludedPlayerNames
local safeAdminEnabled = Config.safeAdminEnabled
local adminPlayers = Config.adminPlayers
...
RunService.RenderStepped:Connect(function()
    ...
    for i = 1, 8 do
        ...
        FOV_Lines[i].Color = Color3.fromHSV(0.75, 1, 1)
    ...
    if CurrentTarget and CurrentTarget.Character then
        ...
        if hum and hum.Health > 0 and targetPart then
            ...
            if onScreen then
                ...
                SA_Tracer.To = tPos
                SA_Tracer.Color = Color3.fromHSV(0.75, 1, 1)
                ...
                for i = 1, 8 do
                    ...
                    Target_Lines[i].Color = Color3.fromHSV(0.75, 1, 1)
...
getgenv().FireRateValue = Config.FireRateValue
getgenv().AccuracyValue = Config.AccuracyValue
getgenv().RecoilValue = Config.RecoilValue
getgenv().DurabilityValue = Config.DurabilityValue
getgenv().AutoValue = Config.AutoValue
getgenv().GunModsAutoApply = Config.GunModsAutoApply

local FistsBuffEnabled = Config.FistsBuffEnabled
...
-- ============================================
local rainbowColors = {
    Color3.fromRGB(128,0,255), Color3.fromRGB(100,50,200), Color3.fromRGB(75,0,130),
    Color3.fromRGB(150,80,255), Color3.fromRGB(180,100,255), Color3.fromRGB(90,30,180), Color3.fromRGB(110,60,220)
}
local BOX_COLOR_MODE = Config.BOX_COLOR_MODE
local BOX_STATIC_COLORS = {
    green = Color3.fromRGB(100,150,255), white = Color3.fromRGB(200,150,255),
    red = Color3.fromRGB(180,80,255), blue = Color3.fromRGB(100,80,255), yellow = Color3.fromRGB(220,150,100),
}

local ShowBoxESP = Config.ShowBoxESP
local PlayerBoxESP = {}
...
local function getBoxColor(espData, lineKey)
    ...
    return BOX_STATIC_COLORS[BOX_COLOR_MODE] or Color3.fromRGB(128,0,255)
end
...
-- ESP ชื่อ
local ShowNameESP = Config.ShowNameESP
local PlayerNameESP = {}
local nameFontSize = Config.nameFontSize
...
RunService.Heartbeat:Connect(function()
    ...
    for player,espData in pairs(PlayerNameESP) do
        ...
        if not espData.character or espData.character ~= player.Character then
...
-- ESP อาวุธ
local ShowItemESP = Config.ShowItemESP
local ItemBillboards = {}
local itemBillboardSize = Config.itemBillboardSize
local WeaponRegistry = {}
local RarityColorsBB = {
    Common=Color3.fromRGB(200,150,255), Uncommon=Color3.fromRGB(150,100,255),
    Rare=Color3.fromRGB(100,80,255), Epic=Color3.fromRGB(128,0,255),
    Legendary=Color3.fromRGB(180,80,255), Omega=Color3.fromRGB(100,50,200)
}
...
local function buildBillboard(player)
    ...
    for _,tool in ipairs(tools) do
        ...
        local rarity=tool:GetAttribute("RarityName") or "Common"
        local rarityColor=RarityColorsBB[rarity] or Color3.new(0.6,0.3,0.8)
        local displayName=tool:GetAttribute("DisplayName") or tool.Name
        if info and info.ImageId and info.ImageId~="rbxassetid://7072725737" then
            ...
            img.BackgroundColor3=Color3.fromRGB(30,20,40)
...
-- ESP ไอเทมบนพื้น
getgenv().ItemESPEnabled = Config.ItemESPEnabled
getgenv().ItemESPMaxDist = Config.ItemESPMaxDist
    ...
    for _,item in ipairs(dropped:GetChildren()) do
        if item:IsA("Model") and item:FindFirstChild("PickUpZone") then
            ...
            if not ItemESPDrawings[item] then 
                ...
                ItemESPDrawings[item].Label.Center = true
                ItemESPDrawings[item].Label.Outline = true
                ItemESPDrawings[item].Label.OutlineColor = Color3.new(0,0,0)
            end
            ...
            if onScreen and dist<getgenv().ItemESPMaxDist then
                ...
                if not color then
                    ...
                    if ok and template then 
                        ...
                        color = RarityColors[rarity] or RarityColors.Common
                    end
...
-- Auto Skip Crate
local AutoSkipEnabled = Config.AutoSkipEnabled
...
local function getRainbowColorForMoney(offset)
    local time = tick() * 2.5
    return Color3.fromHSV(0.75, 1, math.sin(time + offset) * 0.3 + 0.7)
end
...
if _ktype == "adminmax" then
    ...
    AdminTab:Button({Title = "ตรวจสอบสถานะคีย์ปัจจุบัน", Desc = "แสดงข้อมูลคีย์ที่กำลังใช้งานอยู่", Callback = function()
        local s = _load()
        if s and s.key then
...
local BankBalance = PlayerTab:Button({Title = "เงินในธนาคาร", Desc = "<b><font color='#AA66FF'>$0</font></b>", Callback = function() end})
local HandBalance = PlayerTab:Button({Title = "เงินสด", Desc = "<b><font color='#CC88FF'>$0</font></b>", Callback = function() end})
...
task.spawn(function()
    ...
    while true do
        task.wait(0.05)
        hue = hue + direction
        if hue > 0.85 then direction = -0.01
        elseif hue < 0.65 then direction = 0.01 end
...
PlayerTab:Toggle({Title = "เดินเร็ว", Default = Config.walkSpeedEnabled, Callback = function(state) 
    walkSpeedEnabled = state
    Config.walkSpeedEnabled = state
    SaveConfig()
end})
PlayerTab:Slider({Title = "ความเร็ว", Step = 0.01, Value = {Min = 0.01, Max = 0.25, Default = Config.speedValue}, Callback = function(v) 
    speedValue = v
    Config.speedValue = v
    SaveConfig()
end})
...
PlayerTab:Toggle({Title = "กระโดดแรง", Default = Config.jumpEnabled, Callback = function(state)
    jumpEnabled = state
...
PlayerTab:Slider({Title = "ความสูงกระโดด", Step = 5, Value = {Min = 20, Max = 80, Default = Config.jumpPower}, Callback = function(v) 
    jumpPower = v
    Config.jumpPower = v
    SaveConfig()
end})
...
PlayerTab:Toggle({Title = "นั่งใต้ดิน (คงที่)", Desc = "กดเปิดแล้วตัวละครจะนั่งลงใต้ดินตลอดเวลา", Default = Config.sitEnabled, Callback = function(state) toggleSitSystem(state) end})
PlayerTab:Slider({Title = "ความลึก", Step = 0.1, Value = {Min = -2.5, Max = -2, Default = Config.sitHeight}, Callback = function(v) 
    sitHeight = v
    Config.sitHeight = v
    SaveConfig()
    if sitEnabled then toggleSitSystem(true) end
end})

PlayerTab:Divider()
PlayerTab:Section({Title = "ป้องกันการมอง (Anti Look)"})
PlayerTab:Toggle({Title = "เปิดใช้งาน Anti Look", Desc = "ทำให้คนอื่นเห็นตัวละครสั่นและบิดเบี้ยวอย่างรุนแรง", Default = Config.antiLookEnabled, Callback = function(state) 
    toggleFullAntiLook(state)
end})
PlayerTab:Slider({Title = "ความแรงของระบบ", Step = 100, Value = {Min = 500, Max = 5000, Default = Config.shakeStrength}, Callback = function(v)
    setShakeStrength(v)
end})
PlayerTab:Toggle({Title = "โหมดสุ่ม (เพิ่มความแรง)", Desc = "ทำให้การสั่นไม่เป็นจังหวะ เล็งยากขึ้นมาก", Default = Config.randomPatternEnabled, Callback = function(state)
    toggleRandomPattern(state)
end})

PlayerTab:Divider()
PlayerTab:Section({Title = "ป้องกันการตาย"})
PlayerTab:Toggle({Title = "ป้องกันการตาย", Default = Config.antiDeathEnabled, Callback = function(state)
    antiDeathEnabled = state
    Config.antiDeathEnabled = state
    SaveConfig()
...
local vehicleSpeedBoost = Config.vehicleSpeedBoost
local antiRamEnabled = Config.antiRamEnabled
local antiRamDistance = Config.antiRamDistance
local vehicleShakeEnabled = Config.vehicleShakeEnabled
...
VehicleTab:Toggle({Title = "สั่นรถทุกคัน", Default = Config.vehicleShakeEnabled, Callback = function(state)
    vehicleShakeEnabled = state
    Config.vehicleShakeEnabled = state
    SaveConfig()
...
VehicleTab:Slider({Title = "ความเร็ว", Step = 5, Value = {Min = 0, Max = 200, Default = Config.vehicleSpeedBoost}, Callback = function(v) 
    vehicleSpeedBoost = v
    Config.vehicleSpeedBoost = v
    SaveConfig()
end})
...
VehicleTab:Slider({Title = "ระยะป้องกัน", Step = 5, Value = {Min = 20, Max = 80, Default = Config.antiRamDistance}, Callback = function(v) 
    antiRamDistance = v
    Config.antiRamDistance = v
    SaveConfig()
end})
VehicleTab:Toggle({Title = "เปิดใช้ป้องกันการชน", Default = Config.antiRamEnabled, Callback = function(state)
    antiRamEnabled = state
    Config.antiRamEnabled = state
    SaveConfig()
    if state then
        local antiRamConn
...
ESPTab:Toggle({Title = "แสดงกล่อง ESP", Default = Config.ShowBoxESP, Callback = function(v)
    ShowBoxESP = v
    Config.ShowBoxESP = v
    SaveConfig()
...
ESPTab:Dropdown({Title = "สีกล่อง", Values = {"Rainbow","Green","White","Red","Blue","Yellow"}, Default = 1, Callback = function(v) 
    BOX_COLOR_MODE = v:lower()
    Config.BOX_COLOR_MODE = BOX_COLOR_MODE
    SaveConfig()
end})

ESPTab:Section({Title = "ESP ชื่อ"})
ESPTab:Toggle({Title = "แสดงชื่อ", Default = Config.ShowNameESP, Callback = function(v) 
    ShowNameESP = v
    Config.ShowNameESP = v
    SaveConfig()
end})
ESPTab:Slider({Title = "ขนาดตัวอักษร", Step = 1, Value = {Min = 12, Max = 28, Default = Config.nameFontSize}, Callback = function(v) 
    nameFontSize = v
...
ESPTab:Toggle({Title = "แสดงอาวุธ", Default = Config.ShowItemESP, Callback = function(v) 
    ShowItemESP = v
    Config.ShowItemESP = v
    SaveConfig()
    refreshAllBillboards() 
end})
ESPTab:Slider({Title = "ขนาดไอคอน", Step = 2, Value = {Min = 18, Max = 40, Default = Config.itemBillboardSize}, Callback = function(v) 
    itemBillboardSize = v
    Config.itemBillboardSize = v
    SaveConfig()
    if ShowItemESP then refreshAllBillboards() end
end})

ESPTab:Section({Title = "ESP ไอเทมบนพื้น"})
ESPTab:Toggle({Title = "แสดงไอเทมบนพื้น", Default = Config.ItemESPEnabled, Callback = function(v) 
    getgenv().ItemESPEnabled = v
    Config.ItemESPEnabled = v
    SaveConfig()
end})
ESPTab:Slider({Title = "ระยะไกลสุด", Step = 25, Value = {Min = 50, Max = 1000, Default = Config.ItemESPMaxDist}, Callback = function(v) 
    getgenv().ItemESPMaxDist = v
    Config.ItemESPMaxDist = v
    SaveConfig()
end})

ESPTab:Divider()
ESPTab:Section({Title = "ข้ามแอนิเมชั่นกล่อง"})
ESPTab:Toggle({Title = "ข้ามแอนิเมชั่นอัตโนมัติ", Default = Config.AutoSkipEnabled, Callback = function(v) 
    AutoSkipEnabled = v
    Config.AutoSkipEnabled = v
    SaveConfig()
...
PVPTab:Toggle({Title = "เล็งอัตโนมัติ", Default = Config.SilentAimEnabled, Callback = function(v) 
    SilentAimEnabled = v
    Config.SilentAimEnabled = v
    SaveConfig()
    if not v then hideFOV() CurrentTarget = nil end 
end})
PVPTab:Slider({Title = "รัศมี FOV", Step = 10, Value = {Min = 50, Max = 600, Default = Config.FOVRadius}, Callback = function(v) 
    FOVRadius = v
    Config.FOVRadius = v
    SaveConfig()
end})
PVPTab:Dropdown({Title = "เป้าหมาย", Values = {"หัว", "ลำตัว", "หน้าอก"}, Default = 1, Callback = function(v) 
    ...
    Config.SelectedAimPart = SelectedAimPart
    SaveConfig()
end})
PVPTab:Input({Title = "เพื่อนที่ปลอดภัย", Placeholder = "ชื่อผู้เล่น...", Callback = function(v)
    ...
    Config.excludedPlayerNames = excludedPlayerNames
    SaveConfig()
end})
PVPTab:Toggle({Title = "โหมดปลอดภัยสำหรับ Admin", Default = Config.safeAdminEnabled, Callback = function(v) 
    safeAdminEnabled = v
    Config.safeAdminEnabled = v
    SaveConfig()
end})
PVPTab:Input({Title = "รายชื่อ Admin", Placeholder = "ชื่อผู้เล่น...", Callback = function(v)
    ...
    Config.adminPlayers = adminPlayers
    SaveConfig()
end})
...
GunModTab:Toggle({Title = "ปรับอัตโนมัติ", Default = Config.GunModsAutoApply, Callback = function(v) 
    getgenv().GunModsAutoApply = v
    Config.GunModsAutoApply = v
    SaveConfig()
end})
...
GunModTab:Divider()
GunModTab:Slider({Title = "อัตรายิง", Step = 10, Value = {Min = 100, Max = 3000, Default = Config.FireRateValue}, Callback = function(v) 
    getgenv().FireRateValue = v
    Config.FireRateValue = v
    SaveConfig()
end})
GunModTab:Slider({Title = "ความแม่น", Step = 0.01, Value = {Min = 0, Max = 1, Default = Config.AccuracyValue}, Callback = function(v) 
    getgenv().AccuracyValue = v
    Config.AccuracyValue = v
    SaveConfig()
end})
GunModTab:Slider({Title = "แรงดีด", Step = 0.1, Value = {Min = 0, Max = 10, Default = Config.RecoilValue}, Callback = function(v) 
    getgenv().RecoilValue = v
    Config.RecoilValue = v
    SaveConfig()
end})
GunModTab:Slider({Title = "ความทนทาน", Step = 1000, Value = {Min = 1000, Max = 999999, Default = Config.DurabilityValue}, Callback = function(v) 
    getgenv().DurabilityValue = v
    Config.DurabilityValue = v
    SaveConfig()
end})
GunModTab:Toggle({Title = "ยิงอัตโนมัติ", Default = Config.AutoValue, Callback = function(v) 
    getgenv().AutoValue = v
    Config.AutoValue = v
    SaveConfig()
end})
...
GunModTab:Toggle({Title = "หมัดกว้าง", Default = Config.FistsBuffEnabled, Callback = function(v) 
    FistsBuffEnabled = v
    Config.FistsBuffEnabled = v
    SaveConfig()
    checkAndModifyFists() 
end})
...
-- CUSTOM TAB
-- ============================================
local fpsBoosterEnabled = Config.fpsBoosterEnabled
local fpsBoosterMode = Config.fpsBoosterMode
...
local function toggleFPSBooster(state)
    ...
    Config.fpsBoosterEnabled = state
    SaveConfig()
...
CustomTab:Toggle({Title = "เพิ่ม FPS", Default = Config.fpsBoosterEnabled, Callback = function(state) toggleFPSBooster(state) end})
CustomTab:Dropdown({Title = "โหมด", Values = {"โหมด 1", "โหมด 2", "โหมด 3"}, Default = Config.fpsBoosterMode, Callback = function(v)
    fpsBoosterMode = v == "โหมด 1" and 1 or v == "โหมด 2" and 2 or 3
    Config.fpsBoosterMode = fpsBoosterMode
    SaveConfig()
...
local PigHubEffectEnabled = Config.PigHubEffectEnabled
local effectBalls = {}
local effectConnection = nil
local ORBIT_RADIUS = Config.ORBIT_RADIUS
local ORBIT_SPEED = Config.ORBIT_SPEED
...
local function createEffectBalls()
    ...
    for i = 1, 8 do
        ...
        ball.Color = Color3.fromHSV(0.75, 1, 1)
...
local function startEffect()
    ...
    effectConnection = RunService.RenderStepped:Connect(function()
        ...
        for i, ball in ipairs(effectBalls) do
            ...
            local angle = (math.pi * 2 / 8) * (i - 1) + (t * ORBIT_SPEED)
            ...
            local hue = 0.75 + math.sin(t * 1.5 + i) * 0.1
            ball.Color = Color3.fromHSV(math.clamp(hue, 0.65, 0.85), 1, 1)
...
local function togglePigHubEffect(state)
    ...
    Config.PigHubEffectEnabled = state
    SaveConfig()
    if state then startEffect() else removeEffectBalls() end
end

CustomTab:Toggle({Title = "เอฟเฟกต์ PigHub", Default = Config.PigHubEffectEnabled, Callback = function(state) togglePigHubEffect(state) end})
CustomTab:Slider({Title = "รัศมีวงโคจร", Step = 0.5, Value = {Min = 2, Max = 12, Default = Config.ORBIT_RADIUS}, Callback = function(v) 
    ORBIT_RADIUS = v
    Config.ORBIT_RADIUS = v
    SaveConfig()
    if PigHubEffectEnabled then startEffect() end
end})
CustomTab:Slider({Title = "ความเร็ววงโคจร", Step = 0.5, Value = {Min = 0.5, Max = 8, Default = Config.ORBIT_SPEED}, Callback = function(v) 
    ORBIT_SPEED = v
    Config.ORBIT_SPEED = v
    SaveConfig()
    if PigHubEffectEnabled then startEffect() end
end})
...
-- CONFIG TAB
-- ============================================
ConfigTab:Section({Title = "ระบบเซฟค่า"})
ConfigTab:Paragraph({Text = "บันทึกและโหลดค่าตั้งค่าทั้งหมด รวมถึงการตั้งค่าต่างๆ ในเมนู"})

ConfigTab:Button({Title = "บันทึกค่าตั้งค่าปัจจุบัน", Callback = function()
    SaveConfig()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "Config",
        Text = "บันทึกค่าตั้งค่าเรียบร้อย!",
        Duration = 3
    })
end})

ConfigTab:Button({Title = "โหลดค่าตั้งค่าล่าสุด", Callback = function()
    if LoadConfig() then
        -- รีโหลดค่าทั้งหมดจาก Config
        walkSpeedEnabled = Config.walkSpeedEnabled
        speedValue = Config.speedValue
        jumpEnabled = Config.jumpEnabled
        jumpPower = Config.jumpPower
        sitHeight = Config.sitHeight
        if Config.sitEnabled then toggleSitSystem(true) else toggleSitSystem(false) end
        toggleFullAntiLook(Config.antiLookEnabled)
        setShakeStrength(Config.shakeStrength)
        toggleRandomPattern(Config.randomPatternEnabled)
        antiDeathEnabled = Config.antiDeathEnabled
        SilentAimEnabled = Config.SilentAimEnabled
        FOVRadius = Config.FOVRadius
        SelectedAimPart = Config.SelectedAimPart
        excludedPlayerNames = Config.excludedPlayerNames
        safeAdminEnabled = Config.safeAdminEnabled
        adminPlayers = Config.adminPlayers
        getgenv().GunModsAutoApply = Config.GunModsAutoApply
        getgenv().FireRateValue = Config.FireRateValue
        getgenv().AccuracyValue = Config.AccuracyValue
        getgenv().RecoilValue = Config.RecoilValue
        getgenv().DurabilityValue = Config.DurabilityValue
        getgenv().AutoValue = Config.AutoValue
        FistsBuffEnabled = Config.FistsBuffEnabled
        ShowBoxESP = Config.ShowBoxESP
        BOX_COLOR_MODE = Config.BOX_COLOR_MODE
        ShowNameESP = Config.ShowNameESP
        nameFontSize = Config.nameFontSize
        ShowItemESP = Config.ShowItemESP
        itemBillboardSize = Config.itemBillboardSize
        getgenv().ItemESPEnabled = Config.ItemESPEnabled
        getgenv().ItemESPMaxDist = Config.ItemESPMaxDist
        AutoSkipEnabled = Config.AutoSkipEnabled
        vehicleSpeedBoost = Config.vehicleSpeedBoost
        antiRamEnabled = Config.antiRamEnabled
        antiRamDistance = Config.antiRamDistance
        vehicleShakeEnabled = Config.vehicleShakeEnabled
        toggleFPSBooster(Config.fpsBoosterEnabled)
        fpsBoosterMode = Config.fpsBoosterMode
        togglePigHubEffect(Config.PigHubEffectEnabled)
...
ConfigTab:Button({Title = "รีเซ็ตค่าตั้งค่าทั้งหมด", Callback = function()
    Config = {
        walkSpeedEnabled = false,
        speedValue = 0.05,
        jumpEnabled = false,
    ...
    toggleFullAntiLook(false)
    setShakeStrength(2500)
    toggleRandomPattern(false)
    antiDeathEnabled = false
    SilentAimEnabled = false
    FOVRadius = 200
    ...
    togglePigHubEffect(false)
    
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "Config",
        Text = "รีเซ็ตค่าตั้งค่าทั้งหมดเรียบร้อย!",
        Duration = 3
    })
end})
                ...
                for _, obj in ipairs(Window.SideBar:GetDescendants()) do
                    if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                        if obj.Text ~= "" and not obj.Text:find("PIG") then
                            obj.TextColor3 = Color3.fromHSV(0.75, 0.9, 1)
                        end
                    end
                end
            end
