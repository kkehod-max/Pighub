...
-- ========== ปรับค่าให้บินเหนือหัวมอนสเตอร์ ==========
getgenv().TweenSpeed = getgenv().TweenSpeed or 350
getgenv().FarmPos = getgenv().FarmPos or Vector3.new(0, 18, 0)
...
getgenv().AutoClickDelay = getgenv().AutoClickDelay or 0.08
getgenv().AutoClick = getgenv().AutoClick or true

-- ========== AUTO START CONFIG ==========
getgenv().AutoFarm_Level = true
getgenv().AutoFarmNearest = false
getgenv().FastAttack = true
getgenv().SWITCH_TARGET = true

-- ========== FAST ATTACK SPEED BOOST ==========
getgenv().MULTI_HIT = getgenv().MULTI_HIT or 20
getgenv().ATTACK_DELAY = getgenv().ATTACK_DELAY or 0.008
getgenv().DISTANCE = getgenv().DISTANCE or 300

-- ========== OPTIMIZATION ==========
pcall(function()
    settings().Rendering.QualityLevel = 1
    settings().Rendering.EffectsQuality = 1
end)
...
local function VerifyQuest(EnemieName)
    ...
    if not QuestUI.Visible then return false end
    
    local success, title = pcall(function()
        return QuestUI.Container.QuestTitle.Title.Text:gsub("-",""):lower()
    end)
    if not success then return false end
    
    local questName = EnemieName:gsub("-",""):lower()
    return title:find(questName) ~= nil
end
...
local function GetCurrentQuest()
    ...
    if not ok or not QuestUI or not QuestUI.Visible then 
        return nil 
    end
    local success, title = pcall(function()
...
local function EquipTool()
    ...
    -- ถ้าไม่เจอให้ลองหา tool ใดก็ได้
    for _, tool in pairs(b:GetChildren()) do
...
-- ========== ฟังก์ชันรับเควสใหม่ที่แก้ไข ==========
local function StartQuest(questName, questNumber, questPos)
    -- เข้าใกล้ NPC รับเควส
    local plrRP = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
    if plrRP and (plrRP.Position - questPos.p).Magnitude > 10 then
        PlayerTP(questPos)
        task.wait(1)
        return false
    end
    
    -- ตรวจสอบว่าใกล้พอแล้ว
    if plrRP and (plrRP.Position - questPos.p).Magnitude <= 15 then
        -- รอให้ NPC โหลด
        task.wait(0.5)
        
        -- ส่งคำขอรับเควส
        local success, result = pcall(function()
            return FireRemote("StartQuest", questName, questNumber)
        end)
...
-- ฟังก์ชันตรวจสอบว่าเควสสำเร็จหรือยัง
local function IsQuestCompleted()
    ...
    -- ตรวจสอบปุ่ม Complete
    local completeButton = QuestUI:FindFirstChild("Container"):FindFirstChild("Frame"):FindFirstChild("Complete")
    if completeButton and completeButton.Visible then
        return true
    end
    
    -- ตรวจสอบจำนวนมอนสเตอร์ที่ต้องฆ่า
    local needed = QuestUI.Container.QuestTitle.QuestTrack.Required.Text
    local current = QuestUI.Container.QuestTitle.QuestTrack.Current.Text
    
    if needed and current then
        local needNum = tonumber(needed:match("%d+")) or 0
        local curNum = tonumber(current:match("%d+")) or 0
        if curNum >= needNum and needNum > 0 then
            return true
        end
    end
...
-- ฟังก์ชันทำให้ตัวละครบิน
local function FlyAboveGround()
    local plrChar = Player.Character
    if plrChar and plrChar:FindFirstChild("HumanoidRootPart") and getgenv().AutoFarm_Level then
        ...
        if rayResult then
            ...
            local targetY = groundY + 15
            if rootPart.Position.Y < targetY - 2 then
            ...
            if getgenv().AutoFarm_Level then
                ...
                if plrChar then
                    ...
                    if plrChar:FindFirstChild("Humanoid") then
                        plrChar.Humanoid.PlatformStand = true
                        plrChar.Humanoid.AutoRotate = false
                ...
                if plrChar then
                    ...
                    if plrChar:FindFirstChild("Humanoid") then
                        plrChar.Humanoid.PlatformStand = false
                        plrChar.Humanoid.AutoRotate = true
                    end
                end
            end
        end)
...
-- ========== AUTO FARM LEVEL (แก้ไขให้รับเควสได้) ==========
local function AutoFarm_Level()
    while getgenv().AutoFarm_Level do 
        task.wait()
        
        -- ตรวจสอบและส่งเควส (รับรางวัล) ก่อน
        ...
        local hasQuest = VerifyQuest(Quest[3])
        
        if not hasQuest then
            -- ไม่มีเควส ไปรับเควส
            local questPos = Quest[1]
            if Quest[2] and type(Quest[2]) == "CFrame" then
                questPos = Quest[2]
            end
            StartQuest(Quest[4], Quest[5], questPos)
            task.wait(1)
        elseif Enemie and Enemie:FindFirstChild("HumanoidRootPart") then
            -- มีเควสและมีมอนสเตอร์ ไปฆ่า
            local enemyPos = Enemie.HumanoidRootPart.Position
            local flyPos = CFrame.new(enemyPos.X, enemyPos.Y + getgenv().FarmPos.Y, enemyPos.Z)
            PlayerTP(flyPos)
            pcall(function() 
                PlayerClick() 
                ActiveHaki() 
                ...
                BringNPC(Enemie)
                local plrChar = Player.Character
                if plrChar and plrChar:FindFirstChild("HumanoidRootPart") and Enemie:FindFirstChild("HumanoidRootPart") then
                    plrChar.HumanoidRootPart.CFrame = CFrame.new(plrChar.HumanoidRootPart.Position, Enemie.HumanoidRootPart.Position)
                end
            end)
        else
            -- ไม่มีมอนสเตอร์ ให้ไป spawn point
            local plrPP = Player.Character and Player.Character.PrimaryPart
            if QuestTween and QuestTween[1] and plrPP and (plrPP.Position - QuestTween[1].p).Magnitude < 1200 then
                TweenNPCSpawn(QuestTween)
            elseif Quest[2] and type(Quest[2]) == "CFrame" then
                PlayerTP(Quest[2])
            else
                PlayerTP(Quest[1])
            end
...
UIS.InputBegan:Connect(function(i, g)
    ...
    if i.KeyCode == Enum.KeyCode.F1 then
        getgenv().AutoFarm_Level = not getgenv().AutoFarm_Level
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    ...
    Text = "บินเหนือหัวมอนสเตอร์ | รับเควสอัตโนมัติ | F1=ปิด/เปิด",
    Duration = 5
})